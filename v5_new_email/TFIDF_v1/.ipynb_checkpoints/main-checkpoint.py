...
#             help="random seed for np.random.seed, torch.manual_seed and torch.cuda.manual_seed.")
    ...
    parser.add_argument('--data_dir', type=str, default="/opt/omniai/work/instance1/jupyter/v5_new_email/TFIDF_revisit/features/dedup_target")
    ...
    # root_dir="/opt/omniai/work/instance1/jupyter/v5_new_email/TFIDF_v0/"
    ...
    if args.feedback_as_complaint:
        ...
        df_train=pd.read_pickle(os.path.join(args.data_dir,"train_data_v0_"+str(args.max_feature_num)))
        df_val=pd.read_pickle(os.path.join(args.data_dir,"val_data_v0_"+str(args.max_feature_num)))
        # df_test=pd.read_pickle(os.path.join(args.data_dir,"test_data_v0_"+str(args.max_feature_num)))        
    
    ### eliminate the rows with TFIDF vaue==0
    df_train=df_train[df_train.iloc[:,0:990].sum(axis=1)!=0].reset_index(drop=True)
    df_val=df_val[df_val.iloc[:,0:990].sum(axis=1)!=0].reset_index(drop=True)
    # df_test=df_test[df_test.iloc[:,0:990].sum(axis=1)!=0].reset_index(drop=True)
