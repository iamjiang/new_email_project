...
if __name__=="__main__":
    ...
    parser.add_argument("--test_date", type=str, default="08_23", help="the month for test set")
    parser.add_argument("--data_name", type=str, default="train_val_test_dedup_target")
    parser.add_argument("--feedback_as_complaint", action="store_true", help="treat feedback as complaint in training and validation ?")   
    
    parser.add_argument("--start_date", type=str, default='2022-09-01', help="the starting date of time window")
    parser.add_argument("--end_date", type=str, default='2023-09-30', help="the ending date of time window")
    ...
    df=pd.read_pickle(os.path.join(data_path,args.data_name))
    
    begin_row=df.shape[0]
    df=df.dropna(subset=['preprocessed_email'])
    df=df[df.preprocessed_email.notna()]
    df=df[df.preprocessed_email.str.len()>0]
    end_row=df.shape[0]
    print()
    print(f"{end_row-begin_row} rows drop dued to empty text")
    print()
